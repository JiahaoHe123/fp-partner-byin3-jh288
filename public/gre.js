 ...
 * Section: CSE 154 AD
 *
 * This is the js file for the gre.page
 */
"use strict";
(function () {
   ...
   */
  function init() {
    id("mathematics").addEventListener("click", function () {
      display("math");
    });
    id("vocabulary").addEventListener("click", function () {
      display("grammar");
    });
    id("all").addEventListener("click", function () {
      display("");
    });
    id("gre-math-vip-courses").addEventListener("click", function () {
      confirm('math vip courses');
    });
    id("gre-math-courses").addEventListener("click", function () {
      confirm('math courses');
    });
    grammaButton();
    releaseButton();
    fillAll();
  ...
  /**
   * Add the function of gramma for gramma button.
   */
  function grammaButton() {
    id("gre-grammar-vip-courses").addEventListener("click", function () {
      confirm('grammar vip courses');
    });
    id("gre-grammar-courses").addEventListener("click", function () {
      confirm('grammar courses');
    });
  }
  ...
  function display(subject) {
    let courseList = ["math1", "math2", "grammar1", "grammar2"];
  ...
  async function checkPermision(courseName) {
    ...
    if (words.indexOf('vip') !== -1 && words.length >= 3) {
      let course = `gre ${words[0]} ${words[2]}`;
  ...
  /**
   * display the information of successfully re
   * gistration,
  ...
  async function displaySuccess(courseName) {
    ...
    if (qualify) {
      ...
      para.append('course', "gre " + courseName);
      fetch('/course/profile/enrollCourse/gre', {
        method: 'POST',
        body: para
      })
        .then(statusCheck)
  ...
  function displaySuccessEnroll(courseName) {
    ...
    newp.textContent = "You are successfully enrolled in gre " + courseName + "!";
   ...
   */
  function fillAll() {
    let list1 = ["gre-math-vip", "gre-math", "gre-grammar-vip", "gre-grammar"];
  ...
  function fill(name, name1) {
    name = name.replace(/-/g, ' ');
    name += " courses";
    fetch('/gre/course/left?left=' + name)
      .then(statusCheck)
      .then(res => res.json())
      .then(data => {
        fulfill(data, name1);
  ...
  function disableButton() {
    fetch('/course/profile/findCourse/gre')
      .then(statusCheck)
      .then(res => res.json())
      .then(helper)
      .catch(handleError);
